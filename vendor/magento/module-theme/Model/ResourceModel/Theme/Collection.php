    ...
    \Magento\Framework\View\Design\Theme\ListInterface
{
    ...
    {
        //echo "DB2MOD quoteFilter Estoy pasando por aqui 11 " . PHP_EOL;
        $this->_reset()->clear();
        list($area, $themePath) = explode('/', $fullPath, 2);
        $this->addFieldToFilter('area', $area);
        $this->addFieldToFilter('theme_path', $themePath);

        $obj = $this->addFieldToFilter('theme_path', $themePath);
        //var_dump(gettype($obj));
        //var_dump(get_class($obj)); //"Magento\Theme\Model\ResourceModel\Theme\Collection\Intercepto
