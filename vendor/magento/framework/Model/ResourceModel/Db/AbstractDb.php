...
abstract class AbstractDb extends AbstractResource
{
     ...
     * @return \Magento\Framework\DB\Adapter\AdapterInterfaceIbm|false
     */
    ...
    public function load(\Magento\Framework\Model\AbstractModel $object, $value, $field = null)
    {
        ...
        if ($connection && $value !== null) {
            //Investigacion 27/03/2023
            //echo "Aqui estoy 111 " . PHP_EOL;
            //#10 /var/www/html/magento2/vendor/magento/framework/Model/ResourceModel/Db/AbstractDb.php(340): Magento\Framework\DB\Adapter\Pdo\Ibm\Interceptor->fetchRow()
            $select = $this->_getLoadSelect($field, $value, $object); //28/03/2023 INVESTIGAR AQUI
            //var_dump(gettype($select)); //Aqui es un objeto
    ...
    {
        //var_dump($field); //flag_code (Parece bien)
        //var_dump($value); // config_hash
        //var_dump(gettype($object));
        
        //DB2MOD
        //27/03/2023 INVESTIGACIÓN _getLoadSelect 28/03/2023 se requieren mas pruebas

        //var_dump(get_class($this->getConnection())); 
        //Magento\Framework\DB\Adapter\Pdo\Ibm\Interceptor

        //var_dump(sprintf('%s.%s', $this->getMainTable(), $field)); //Solamente pega dos campos flag.flag_code
        //var_dump($this->getConnection()->quoteIdentifier("hola.hola")); //Metodo responsable

        //DB2MOD FIX, 28/03/2023 LINEA ANTERIOR:
        /*$field = $this->getConnection()->quoteIdentifier(sprintf('%s.%s', $this->getMainTable(), $field));*/
        //DB2MOD FIX, 28/03/2023 LINEA NUEVA:
        $field = sprintf('%s.%s', $this->getMainTable(), $field);

        //var_dump(gettype($field));
        //var_dump($field); //DB2MOD 28/03/2023 Comillas dobles- VIENE DESDE ATRAS PERO ES AQUI
        $select = $this->getConnection()->select()->from($this->getMainTable())->where($field . '=?', $value);
        //var_dump(gettype($select));
        return $select;
    }
