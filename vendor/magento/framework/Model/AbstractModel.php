...
abstract class AbstractModel extends DataObject
{
    ...
    public function getId()
    {
        ...
        //echo "DB2MOD PASS_1 INTERCEPTOR GROUPREP";
        //CAMBIO HECHO EN vendor/magento/framework/DataObject 21/02/2023
        
        //DB2MOD IMPORTANTE YA QUE DB2 HACE TODO EN MAYUSCULAS
        $keyField = "";

        if(gettype($this->_idFieldName) == "string" && $this->_idFieldName != NULL){
            $keyField = strtoupper($this->_idFieldName);
        }
        //var_dump($keyField);

        /*if($keyField == "GROUP_ID"){
            var_dump($this->_getData($keyField));
        }*/

        //DB2MOD FIX
        if($keyField != ""){
            $varReturn = $this->_getData($keyField);
            if($varReturn != NULL){
                return $this->_getData($keyField);
            }  
        }
