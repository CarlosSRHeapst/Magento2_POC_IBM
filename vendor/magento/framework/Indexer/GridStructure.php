...
class GridStructure implements IndexStructureInterface
{
    ...
    protected function createFlatTable($tableName, array $fields)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($field['type'] === 'filterable' || (isset($field['index']) && $field['index'] === true)) {
                $table->addIndex(
                    $this->resource->getIdxName($tableName, $name, AdapterInterfaceIbm::INDEX_TYPE_INDEX),
                    $name,
                    ['type' => AdapterInterfaceIbm::INDEX_TYPE_INDEX]
                );
            }
        ...
        $table->addIndex(
            $this->resource->getIdxName(
                $tableName,
                $searchableFields,
                AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT
            ),
            $searchableFields,
            ['type' => AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT]
        );
     ...
     *
     * @return false|AdapterInterfaceIbm
     */
