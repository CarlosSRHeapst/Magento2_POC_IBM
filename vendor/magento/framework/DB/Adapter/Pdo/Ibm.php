<?php
/**
 * Copyright © Heapstash S.A. de C.V. All rights reserved.
 * Creado por Carlos A. Sánchez para utilizar una base de datos IBM-DB2 en Magento
 * Inicio 09 12 2022 NO ESTA LISTA
 ...
 * @since 100.0.2
 */
class Ibm extends \Zend_Db_Adapter_Pdo_Ibm implements AdapterInterfaceIbm
{
    ...
    public const DDL_CACHE_PREFIX      = 'DB_PDO_IBM_DDL';
    public const DDL_CACHE_TAG         = 'DB_PDO_IBM_DDL';
     ...
     * @var string
     */
    protected $_defaultStmtClass = \Magento\Framework\DB\Statement\Pdo\Ibm::class;
    ...
    protected $_ddlColumnTypes      = [
        ...
        //Se manejara timestamp como DATETIME ya que la palabra no existe en DB2, timestamp puede 
        //recibir un string de tiempo yyyy-mm-dd hh:mm:ss
        Table::TYPE_DATETIME      => 'timestamp',
    ...
    public function __construct(
        ...
        /*$this->exceptionMap = [
            923 => ConnectionException::class, //CONNECTION NOT ESTABLISHED: DB2 condition REASON reason-code, TYPE resource-type, NAME resource-name
            924 => ConnectionException::class, //DB2 CONNECTION INTERNAL ERROR, function-code,return-code,reason-code
            30082 => ConnectionException::class,// SQLSTATE[HY000]: General error: 2006 MySQL server has gone away
            4296 => ConnectionException::class  // Exception {0}: Error opening socket to server {1} on port {2} with message: {3}. ERRORCODE=-4296, SQLSTATE=08001
        ];*/
        try {
    ...
    public function beginTransaction()
    {
        if ($this->_isRolledBack) {
            ...
            throw new \Exception(AdapterInterfaceIbm::ERROR_ROLLBACK_INCOMPLETE_MESSAGE);
        }
    ...
    public function commit()
    {
        if ($this->_transactionLevel === 1 && !$this->_isRolledBack) {
            ...
            // phpcs:ignore Magento2.Exceptions.DirectThrow.FoundDirectThrow
            throw new \Exception(AdapterInterfaceIbm::ERROR_ASYMMETRIC_COMMIT_MESSAGE);
    ...
    public function rollBack()
    {
        if ($this->_transactionLevel === 1) {
            ...
            // phpcs:ignore Magento2.Exceptions.DirectThrow.FoundDirectThrow
            throw new \Exception(AdapterInterfaceIbm::ERROR_ASYMMETRIC_ROLLBACK_MESSAGE);
        } else {
     ...
     */
    //DB2MOD Magento al compilar lo exigio publico
    ...
    public function _connect()
    {
        ...
        //Cambia de pdo_mysql a solo PDO (tambien considerar "ibm_db2")
        if (!extension_loaded('pdo')) {
            throw new Zend_Db_Adapter_Exception('pdo extension is not installed');
        }
        ...
        //Se comenta, no es valido en DB2
        //$this->_connection->query("SET SQL_MODE=''");

        // As we use default value CURRENT_TIMESTAMP for TIMESTAMP type columns we need to set GMT timezone
        //Se comenta DB2 obtiene la zona de tiempo desde el sistema
        //$this->_connection->query("SET time_zone = '+00:00'");


        // Ya deberia tener la codificacion UTF8 desde la base de datos DB2
        //https://stackoverflow.com/questions/27153521/db2-how-to-specify-connection-charset-using-pdo
        //Se debe quitar initStatements de app/etc/env.php
        //Esta linea es para que se pongan los queries que quieran utilizarse al arrancar, en este caso
        //mysql cambiaba sus interacciones a UTF8
        ...
        /*Define los queries en buffer en DB los determina por espacio de memoria:
        ...
        ATTR_EMULATE_PREPARES parece ser unico de Mysql
        */
        ...
        //27/03/2023 Atributo para columnas sacado de la DOC de PDO.
        $this->_connection->setAttribute(\PDO::ATTR_CASE, \PDO::CASE_LOWER);

        //Sin el backspace:
        /*
        Class "Magento\Framework\DB\Adapter\Pdo\PDO" not found#0 
        /var/www/html/magento2/generated/code/Magento/Framework/DB/Adapter/Pdo/Ibm/Interceptor.php(77): 
        Magento\Framework\DB\Adapter\Pdo\Ibm->_connect() 
        */

        //Con el backspace
        /*
         Warning: Undefined array key "CODE" in /var/www/html/magento2/vendor/magent  
        o/module-store/App/Config/Source/RuntimeConfigSource.php on line 111 
        */
    ...
    protected function _checkDdlTransaction($sql)
    {
        if ($this->getTransactionLevel() > 0) {
            ...
            if (in_array($startSql, $this->_ddlRoutines) && strcasecmp($sqlMessage[1], 'temporary') !== 0) {
                throw new ConnectionException(AdapterInterfaceIbm::ERROR_DDL_MESSAGE, E_USER_ERROR);
            }
        }
    }
    ...
    {
        //DB2MOD aqui todavia es objeto var_dump($sql); // Antiguo

        //27/03/2023 flag
        //28/03/2023 request_path
        //SQL OBJETO
        /*var_dump(gettype($sql));
        var_dump(get_class($sql)); //Select crudos
        var_dump(get_class_methods($sql));
        var_dump(get_object_vars($sql));*/
        ...
        $connectionErrors = [
            ...
            4296,  // Exception {0}: Error opening socket to server {1} on port {2} with message: {3}. ERRORCODE=-4296, SQLSTATE=08001
        ];
        $triesCount = 0;
        do {
            $retry = false;
            ...
            try {
                ...
                $this->_prepareQuery($sql, $bind); //DB2MOD MUY IMPORTANTE OBJETO ENCARGADO DE STRINGIFY


                //DB2MOD aqui ya esta hecho string var_dump($sql);
                //$result es la ultima llamada


                //27/03/2023 
                /* ENTRE
                #4 /var/www/html/magento2/vendor/magento/zendframework1/library/Zend/Db/Adapter/Pdo/Abstract.php(260): Zend_Db_Adapter_Abstract->query()
                #5 /var/www/html/magento2/vendor/magento/framework/DB/Adapter/Pdo/Ibm.php(608): Zend_Db_Adapter_Pdo_Abstract->query()
                */
                //Aqui ya es string
                //var_dump($sql);
                ...
                if (!$retry) {
                    ...
                    if ($pdoException && isset($this->exceptionMap[$pdoException->errorInfo[1]])) {
                        /**
                         *Hasta ahora internamente no sabemos como funcionan las excepciones de DB2 y cuales son las mas comunes 
                         *por lo tanto los valores deexceptionMap se comentan pero la variable se deja indefinida para que no se entre aqui
                         *Lo que hay que checar es errorInfo
                         *$customExceptionClass = $this->exceptionMap[$pdoException->errorInfo[1]];
                        */

                        /**  
                         * este llevaba un arroba antes del var
    ...
    {
        //DB2MOD
        //DB2MOD 27/03/2023 problema de flag
        //var_dump(gettype($sql)); //Aqui es un objeto
        //var_dump(get_class($sql));
        $sql = (string) $sql;
        //var_dump($sql); // Aqui se hace el cast directo FLAG esta mal.
        //var_dump($sql);
        if (!is_array($bind)) {
            $bind = [$bind];
        }
    ...
    public function purgeOrphanRecords(
        ...
        $refTableName,
        $refColumnName,
        $onDelete = AdapterInterfaceIbm::FK_ACTION_CASCADE
    ) {
        $onDelete = strtoupper($onDelete);
        if ($onDelete == AdapterInterfaceIbm::FK_ACTION_CASCADE
            || $onDelete == AdapterInterfaceIbm::FK_ACTION_RESTRICT
        ) {
            $sql = sprintf(
            ...
            $this->rawQuery($sql);
        } elseif ($onDelete == AdapterInterfaceIbm::FK_ACTION_SET_NULL) {
            $sql = sprintf(
    ...
    public function showTableStatus($tableName, $schemaName = null)
    {
        ...
        /*
        En DB2 el equivalente al chequeo del estatus de la table seria:
        SELECT * FROM SYSIBMADM.ADMINTABINFO where TABSCHEMA = 'MAGENTO2' AND TABNAME = 'ADOBE_STOCK_ASSET';
        Por ejemplo, sin embargo este tarda por lo menos 10 segundos por consulta de tabla y MYSQL tarda 1
        */
        $fromDbName = null;
        if ($schemaName !== null) {
        ...
        foreach ($tables as $table => $tableData) {
            ...
            foreach ($tableData['columns'] as $column => $columnDefinition) {
                ...
                foreach ($droppedKeys as $options) {
                    ...
                    if ($onDelete == AdapterInterfaceIbm::FK_ACTION_SET_NULL) {
                    ...
                    $this->addForeignKey(
                        ...
                        ($onDelete) ? $onDelete : AdapterInterfaceIbm::FK_ACTION_NO_ACTION
                    );
                }
            }
    ...
    public function getIndexList($tableName, $schemaName = null)
    {
        ...
        if ($ddl === false) {
            ...
            foreach ($this->fetchAll($sql) as $row) {
                ...
                if (strtolower($row[$fieldKeyName] ?? '') == AdapterInterfaceIbm::INDEX_TYPE_PRIMARY) {
                    $indexType  = AdapterInterfaceIbm::INDEX_TYPE_PRIMARY;
                } elseif ($row[$fieldNonUnique] == 0) {
                    $indexType  = AdapterInterfaceIbm::INDEX_TYPE_UNIQUE;
                } elseif (strtolower($row[$fieldIndexType] ?? '') == AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT) {
                    $indexType  = AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT;
                } else {
                    $indexType  = AdapterInterfaceIbm::INDEX_TYPE_INDEX;
                }
    ...
    public function createTableByDdl($tableName, $newTableName)
    {
        ...
        foreach ($indexes as $indexData) {
            ...
            if (($indexData['KEY_NAME'] == 'PRIMARY')
                || ($indexData['INDEX_TYPE'] == AdapterInterfaceIbm::INDEX_TYPE_PRIMARY)
            ) {
                continue;
            }
    ...
    protected function _getIndexesDefinition(Table $table)
    {
        ...
        foreach ($indexes as $indexData) {
            if (!empty($indexData['TYPE'])) {
                ...
                if (($indexData['TYPE'] == AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT) && $this->isNdb($table)) {
                    continue;
                }
                switch ($indexData['TYPE']) {
                    case AdapterInterfaceIbm::INDEX_TYPE_PRIMARY:
    ...
    {
        //27/03/2023 Analisis de conveniencia, las llaves ddl se guardaran en minusculas.
        //A reservas de futuros errores o problemas
        $cacheKey = $this->_getTableName($tableName, $schemaName);
        ...
        //27/03/2023 PARA IBM DB2 EL ESQUEMA DEFAULT SERA MAGENTO2
        //$fromDbName = 'DATABASE()';
        $fromDbName = 'MAGENTO2';
        if ($schemaName !== null) {
        ...
        //Se detecto un conflicto de SQL, en 
        ...
        $sql = sprintf(
            'SELECT COUNT(1) AS tbl_exists FROM SYSCAT.TABLES WHERE TABNAME = %s AND TABSCHEMA = %s',
            $this->quote($tableName),
            $this->quote($fromDbName)
        );
        $ddl = $this->rawFetchRow($sql, 'tbl_exists');
        if ($ddl) {
            //27/03/2023 IBM DB2 MOD - Checa que exista la tabla y guarda en cache el acceso (minusculas a 27 MAR)
    ...
    public function addIndex(
        ...
        if (isset($keyList[strtoupper($indexName)])) {
            if ($keyList[strtoupper($indexName)]['INDEX_TYPE'] == AdapterInterfaceIbm::INDEX_TYPE_PRIMARY) {
        ...
        switch (strtolower((string)$indexType)) {
            case AdapterInterfaceIbm::INDEX_TYPE_PRIMARY:
                ...
                break;
            case AdapterInterfaceIbm::INDEX_TYPE_UNIQUE:
                $condition = 'UNIQUE ' . $this->quoteIdentifier($indexName);
                break;
            case AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT:
    ...
    {
        //DB2MOD DB2 exige una tabla de donde convertir no es seguro que funcione

        //select DATE(TO_DATE(CRDTR2, 'YYYY/MM/DD'))
    ...
    public function getDateExtractSql($date, $unit)
    {
        ...
        //DB2MOD Aqui parece que DB2 EXIGE una tabla de donde sacar sus resultados no solo lo hace standalone 
    ...
    public function getIndexName($tableName, $fields, $indexType = '')
    {
        ...
        switch (strtolower((string)$indexType)) {
            ...
            /*case AdapterInterfaceIbm::INDEX_TYPE_FULLTEXT:
                $prefix = 'fti_';
                break;*/
            case AdapterInterfaceIbm::INDEX_TYPE_INDEX:
            default:
                $prefix = 'idx_';
        }
    ...
    {
        //DB2MOD Falta el schema, se debe declarar el esquema donde esta la tabla
        ...
        //DB2MOD SE COLOCA ESQUEMA
        $insertSql = sprintf('INSERT %s INTO MAGENTO2.%s (%s) VALUES %s', $strategy, $tableName, $columns, $values);
    ...
    {
        //Se cambian variables para identificar las iniciales de los implodes
        $tableNameN = $this->quoteIdentifier($tableName, true);

        
        $columnsQ   = array_map([$this, 'quoteIdentifier'], $columns);

        $columnsIMP   = implode(',', $columnsQ);
        $columnsIMP_S   = implode(',', $columns);
        $valuesIMP    = implode(', ', $values);

        /*
        DB2MOD REPLACE into no existe en DB2 la sintaxis es mas compleja
        Hay que probarla multiples veces se desconoce si el match es perfecto
        */
        /*
        Variable para hacer el update en el merge de DB2 (Siendo el REPLACE INTO anterior de MYSQL
        el cual hace una seleccion, si encuentra el VALOR buscado lo actualiza con UPDATE y sin no lo encuentra
        lo inserta con el comando INSERT, sin embargo en DB2 esto debe ser MANUAL)
        */
        $updateSet = "";
        $updateSetValuesMerge = "";
        $totalColumnas = count($columns);

        $PK_FirstCol = $columns[0];

        //Foreach para empatar tabla temporal merge con las columnas de la tabla actual
        for($i = 0; $i <= $totalColumnas ; $i++){
             if($i != 0 && $i != $totalColumnas ){
                $updateSet = $updateSet . ',';
                $updateSetValuesMerge = $updateSetValuesMerge . ',';
             }

             $updateSet = $updateSet . ' ' . $tableName . '.' . $columns[$i] . ' = merge.' . $columns[$i];
             $updateSetValuesMerge = $updateSetValuesMerge . 'merge.' . $columns[$i];
        }

        //$replaceSql = sprintf('REPLACE INTO %s (%s) VALUES %s', $tableName, $columns, $values);

        $replaceSql = sprintf(
            'MERGE INTO MAGENTO2.%1$s AS tab
             USING (VALUES (%2$s)) AS merge (%3$s)
             ON tab.%4$s = merge.%4$s
             WHEN MATCHED THEN
                UPDATE SET %5$s
            WHEN NOT MATCHED THEN
                INSERT (%3$s)
                VALUES (%6$s)', $tableName, $valuesIMP, $columnsIMP_S, $PK_FirstCol, $updateSet, $updateSetValuesMerge);
     ...
     * @return string
     */
    /*DB2MOD Parece bien pero TYPE y COLUMN TYPE no se puede obtener por query, aqui parece
    ser parte del arreglo unicamente
    */
    ...
    {
        switch ($action) {
            case AdapterInterfaceIbm::FK_ACTION_CASCADE:
                return Table::ACTION_CASCADE;
            case AdapterInterfaceIbm::FK_ACTION_SET_NULL:
                return Table::ACTION_SET_NULL;
            case AdapterInterfaceIbm::FK_ACTION_RESTRICT:
    ...
    protected function _prepareSqlDateCondition($condition, $key)
    {
        if (empty($condition['date'])) {
            // DB2MOD POR IBM se agrega por timestamp
            if (empty($condition['datetime']) || empty($condition['timestamp'])) {
    ...
    public function createTrigger(\Magento\Framework\DB\Ddl\Trigger $trigger)
    {
        ...
        //DB2MOD LA SINTAXIS PARECE COMPATIBLE
    ...
    public function dropTrigger($triggerName, $schemaName = null)
    {
        ...
        //DB2MOD SE PONE UN TRY CATCH y se retira el IF EXISTS porque no existe en DB2
        try{

            $sql = 'DROP TRIGGER ' . $this->quoteIdentifier($triggerName);
            $this->query($sql);
    ...
    {
        /*DB2MOD Estas sentencias no sirven en DB2
        La correcta es SELECT TABLE_NAME FROM SYSIBM.tables WHERE TABLE_SCHEMA = 'MAGENTO2' AND TABLE_NAME LIKE '%CATEGORY%';
        Se pudiera agregar la condicion de "TABLE_SCHEMA" como variable de este metodo, supone hacer un cambio en todas las llamadas
        */
        //$sql = ($likeCondition === null) ? 'SHOW TABLES' : sprintf("SHOW TABLES LIKE '%s'", $likeCondition);
        $sql = ($likeCondition === null) ? "SELECT TABLE_NAME FROM SYSIBM.tables WHERE TABLE_SCHEMA = 'MAGENTO2'" : sprintf("SELECT TABLE_NAME FROM SYSIBM.tables WHERE TABLE_SCHEMA = 'MAGENTO2' LIKE '%s'", $likeCondition);
        $result = $this->query($sql);
        $tables = [];
