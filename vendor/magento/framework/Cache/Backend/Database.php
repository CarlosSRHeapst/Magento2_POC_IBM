...
class Database extends \Zend_Cache_Backend implements \Zend_Cache_Backend_ExtendedInterface
{
    ...
    public function __construct($options = [])
    {
        ...
        if (empty($this->_options['adapter_callback'])) {
            if (!$this->_options['adapter'] instanceof \Magento\Framework\DB\Adapter\AdapterInterfaceIbm) {
                \Zend_Cache::throwException(
                    'Option "adapter" should be declared and extend \Magento\Framework\DB\Adapter\AdapterInterfaceIbm!'
                );
            }
        }
    ...
    protected function _getConnection()
    {
        if (!$this->_connection) {
            ...
            if (!$connection instanceof \Magento\Framework\DB\Adapter\AdapterInterfaceIbm) {
                \Zend_Cache::throwException(
                    'DB Adapter should be declared and extend \Magento\Framework\DB\Adapter\AdapterInterfaceIbm'
                );
            } else {
     ...
     * @throws \Zend_Cache_Exception
     */
    private function cleanAll(\Magento\Framework\DB\Adapter\AdapterInterfaceIbm $connection)
    {
        if ($this->_options['store_data']) {
     ...
     * @throws \Zend_Cache_Exception
     */
    private function cleanOld(\Magento\Framework\DB\Adapter\AdapterInterfaceIbm $connection)
    {
        if ($this->_options['store_data']) {
