...
class Zend_Db_Statement_Pdo extends Zend_Db_Statement implements IteratorAggregate
{
    ...
    protected function _prepare($sql)
    {
        try {
            ...
            $sinFrom   = "FROM";
            $containsFrom = strpos($sql, $sinFrom);

            $singleQuote   = "`";
            $containsSingleQuote = strpos($sql, $singleQuote);

            //Remover cualquier caracter ` ya que no es aceptado en DB2
            if ($containsSingleQuote === false) {
                //DB2MOD si sale el valor "false" es que no se encontro, si se encontro saldra el indice
            }else{
                $sql = str_replace($singleQuote,"",$sql);
            }

            //PRUEBA CON FROM (error de pointer cuando se adjunta SET SCHEMA)
            /*if ($containsFrom === false) {
            }else{
                $sql = str_replace($sinFrom,"FROM MAGENTO2.",$sql);
                $sql = str_replace("FROM MAGENTO2. ","FROM MAGENTO2.",$sql);
            }*/

            //$sql = "SET SCHEMA MAGENTO2; " . $sql . ";";

            //echo "Statement PDO query en prepare " . $sql . PHP_EOL;
            /////echo " Despues " . PHP_EOL;
            //var_dump($sql);

/////////////////////////////////////////////////////////////////////////////////////
            //28/03/2023 error: flag
            //29/03/2023 error: request_path
            //29/03/2023 error: area, theme_path
            /*$needle   = 'theme_path';
            ...
            //27/02/2023 Se encuentra discrepancia entre MYSQL y IBM DB2
            /* 
                El query "SELECT COUNT(1) AS tbl_exists FROM INFORMATION_SCHEMA.TABLES 
                WHERE TABLE_NAME = 'setup_module' AND TABLE_SCHEMA = DATABASE()"

                No se puede replicar en DB2 ya que information_schema no existe como consulta.

                Se debe cambiar por SELECT * FROM SYSCAT.TABLES; (framework/DB/Adapter/Pdo/Ibm.php - Aprox linea 2753)
            */
