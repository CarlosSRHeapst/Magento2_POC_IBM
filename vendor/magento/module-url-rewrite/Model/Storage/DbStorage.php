...
class DbStorage extends AbstractStorage
{
    ...
    protected function prepareSelect(array $data)
    {
        ...
        foreach ($data as $column => $value) {
            //DB2MOD Posible error con quoteidentifier
            //var_dump($this->connection->quoteIdentifier($column) . ' IN (?)');
            //var_dump($column);
            //DB2MOD LINEA ORIGINAL:
    ...
    protected function doFindOneByData(array $data)
    {
        if (array_key_exists(UrlRewrite::REQUEST_PATH, $data)
            ...
            //DB2MOD 29/03/2023 INVESTIGACION request_path
            //STACKTRACE OCULTO POR $this->prepareSelect($data)
            //var_dump($this->prepareSelect($data)); //Obtiene select crudo
    ...
    {
        echo "DB2MOD quoteIdentifier Estoy pasando aqui 10 " . PHP_EOL;
        $oldUrlsSelect = $this->connection->select();
        $oldUrlsSelect->from(
            $this->resource->getTableName(self::TABLE_NAME)
        );
        //DB2MOD $this->connection->quoteIdentifier
