...
class UninstallCollectorTest extends \PHPUnit\Framework\TestCase
{
    ...
    /**
     * @var \PHPUnit\Framework\MockObject\MockObject|\Magento\Framework\DB\Adapter\AdapterInterfaceIbm
     */
    private $AdapterInterfaceIbm;
    ...
    protected function setUp(): void
    {
        ...
        $this->AdapterInterfaceIbm = $this->getMockForAbstractClass(
            \Magento\Framework\DB\Adapter\AdapterInterfaceIbm::class,
            [],
            '',
            false
        );
        $select = $this->createPartialMock(\Magento\Framework\DB\Select::class, ['from']);
        $this->AdapterInterfaceIbm->expects($this->once())->method('select')->willReturn($select);
        $setup->expects($this->exactly(2))->method('getConnection')->willReturn($this->AdapterInterfaceIbm);
    ...
    public function testUninstallCollector()
    {
        ...
        $this->AdapterInterfaceIbm->expects($this->once())
            ->method('fetchAll')
            ->with($this->result)
